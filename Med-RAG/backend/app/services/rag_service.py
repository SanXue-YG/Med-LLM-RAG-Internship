...
class RagService:
    ...
    def _default_factory(self) -> PipelineLike:
        ...
        idx = index_ready(self.config.retrieval_mode)
        if not idx.get("ready"):
            raise AppException(
                ErrorCode.PIPELINE_FAILED,
                message=(
                    "retrieval index not ready — place sample chroma + chunks under "
                    "Med-RAG/data/ or use POST /api/v1/ingest/upload"
                ),
                detail=idx,
            )
        ...
        if backend == "medical08":
            raise AppException(
                ErrorCode.PIPELINE_FAILED,
                message="medical08 backend not wired; use constrained10",
