...
> 当前状态：**进行中** — 阶段 0~3、2.0 已完成；**明日阶段 4**（框架已搭：`domain_analysis.py` + notebook §4）
...
**全量期工程约定**（阶段 2 写代码时预留，阶段 B 再改配置即可）：

- 用 **单一根路径变量** 切换数据源，避免硬编码：
  - 环境变量 `MED_RAG_DATA_ROOT`（推荐），或 notebook / `src/config.py` 中的 `DATA_ROOT`
- **验证期**：`DATA_ROOT = 02 数据处理/data`（`processed/sample.jsonl` 为本工程自有副本）
- **全量期**：`DATA_ROOT = /Volumes/<外接盘名>/med-rag-pmc`（示例），其下建议布局：
  ```text
  <外接盘>/med-rag-pmc/
  ├── raw/              # NCBI 下载的 oa_comm/*.tar.gz
  ├── extracted/        # 解压后的 XML（流式处理后可删批次）
  └── processed/        # 全量 jsonl、清洗结果、统计缓存
  ```
- pipeline **只读** `f"{DATA_ROOT}/processed/*.jsonl"`（或 glob），下载/解压脚本写入 `raw/`、`extracted/`，与加载逻辑分离

**阶段 B 工作流（备忘，代码验证通过后再做）**：

1. 外接硬盘挂载，创建 `med-rag-pmc/` 目录树  
2. 将 `MED_RAG_DATA_ROOT` 指向外接盘（或在 notebook 配置 cell / 环境变量里改一行）  
3. 按批次下载 `oa_comm/xml/`（或 baseline）→ `raw/` → 解压 → **`./scripts/build_jsonl.sh`**（`PMC_XML_ROOT` 指向外接盘）→ `processed/*.jsonl`  
4. 用**同一套**分析 notebook / 脚本对全量再跑 §1~§4（说明文档区分「100 篇验证」与「全量结论」）

> 100 篇足以完成老师要求的分析**方法论**与交付结构；全量用于**统计更稳、缺失率与分位数更可信**，二者在说明文档中分节呈现即可。
...
| 4 | 领域内容理解（任务 §2） | 1 天 | 📋 | 框架已留；明日实现分层样例 + 术语/结构 |
...
│   └── processed/
│       ├── sample.jsonl        # 100 篇（自 01 复制，验证期）
│       └── sample_clean.jsonl  # 清洗后（若有）
│   # 全量期：改 MED_RAG_DATA_ROOT 指向外接盘，不强制搬动本目录
├── scripts/
│   └── build_jsonl.sh          # XML → jsonl 入口
├── src/
│   ├── parse_pmc.py            # JATS 解析
│   ├── build_jsonl.py          # 批量构建
│   └── load_pipeline.py        # jsonl → Dataset
├── notebooks/
...
- [ ] 全量期：`PMC_XML_ROOT` / `MED_RAG_DATA_ROOT` 指向外接盘后同脚本批处理
...
> 与「是否扩大样本」无关：目标是 **全库** 评估，不是再多下几个 incr 小包。

- [ ] 外接硬盘就绪，`MED_RAG_DATA_ROOT` 指向 `<盘>/med-rag-pmc`
- [ ] 下载脚本：列出 `oa_comm/xml/`（及必要时 baseline）→ 批次 `wget`/`requests` 到 `raw/`
- [ ] 流式 pipeline：解压 → `parse_pmc_xml`（含 title/pmid 修复）→ 追加 `processed/oa_comm_full.jsonl`（或分片 jsonl）
- [ ] **不改分析代码**：仅切换 `DATA_ROOT` + 输入文件名，复跑阶段 3~6
- [ ] 说明文档增加一节：**「100 篇验证结论」vs「全量统计结论」** 对比与差异说明
...
> **明日开工**。代码框架：`src/domain_analysis.py` + notebook `§4`（取消注释即可串联）。

### 明日第一步（建议顺序）

1. 【前置 1/2】【前置 2/2】
2. 实现 `domain_analysis.load_tokenizer` / `add_abstract_token_len`
3. notebook §4.1 取消注释 → 生成 `outputs/samples/stratified_*.md`
4. 实现 §4.2 统计 → 填写 §4.3 人工观察表 → 更新说明文档 §4

### 4.1 分层抽样（按 token）

- [ ] 用 **`all-MiniLM-L6-v2` tokenizer** 对 **`abstract`** 计 token（与 §5 同模型，§4 先加载一次即可）
- [ ] 划分短 / 中 / 长（示例：P33 以下 / P33~P66 / P66 以上，或以固定阈值如 100/300 tokens，**以实测分布为准**）
- [ ] 每个区间抽 **5~10 篇**，导出 `outputs/samples/stratified_*.md` 或 notebook 展示

### 4.2 结构与术语（人工 + 简单统计）

- [ ] **IMRaD**：摘要是否常见 Background / Methods / Results / Conclusion 结构（关键词或段落）
- [ ] **缩写**：抽样统计大写缩写密度（如 EGFR、PCI）
- [ ] **同义表述**：记录 1~2 个例子（如 heart attack vs myocardial infarction）
- [ ] （可选）摘要 **词频 / 高频词** Top-30（过滤停用词）

### 4.3 产出

- [ ] 说明文档 §「领域语言特性」：语言风格、信息密度、对 prompt/评估的启示（心理基线，不需写具体 prompt）
