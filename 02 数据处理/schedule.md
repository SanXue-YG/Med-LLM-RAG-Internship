...
> 当前状态：**验证期 A（§1~§6）已完成**；**明日：阶段 B 外接盘全量下载 + 策略验证**；阶段 7 交付整理可与 B 并行
...
- [ ] [PMC OA FTP](https://www.ncbi.nlm.nih.gov/pmc/tools/openftlist/) → `oa_comm/xml/` 清单
...
- [ ] notebook 设 `MED_RAG_JSONL` 后可复跑 §5/§6（抽样）


自用提交：
git add -A
git status
git commit -m "02: 阶段5收尾与文档对齐；schedule/README/说明文档§6初稿"
git push