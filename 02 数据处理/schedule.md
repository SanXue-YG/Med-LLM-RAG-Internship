...
> 当前状态：**进行中** — 阶段 **0~5** 已完成（含任务书 §1~§3 与说明文档 §5.2 定稿）；**阶段 6**（任务书 §4 分割策略）进行中
...
| 6 | 分割策略设计（任务 §4） | 0.5 天 | 🔄 | 决策表 + 可选 LangChain demo |
...
| 条件（基于阶段 5 验证期 97 篇） | 策略 | 工具/参数（建议） |
|---|---|---|
| 约 **85%** 篇 title+abstract **≤512**（P50≈374） | **主体**：单 chunk 整块嵌入 | 检索单元 = **title+abstract**；`chunk_size` 与模型 max（512）对齐 |
| **~14%** 篇 title+abstract **>512**（P95≈617，P99≈1009） | **长尾**：截断或重叠滑动窗口 | `RecursiveCharacterTextSplitter`；`chunk_size=300~500`, `overlap=50~100` |
| 摘要极少显式 IMRaD 小标题（阶段 4） | **优先**滑动窗口；章节切分作备选 | 自定义分隔符仅在有明确 `METHODS:`/`RESULTS:` 等时考虑 |
