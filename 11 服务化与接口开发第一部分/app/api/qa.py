    ...
    try:
        ...
        store.append(
            ...
            SessionTurn(
                query=body.query,
                answer=answer,
                meta={"top_k": body.top_k, "n_sources": len(sources)},
            ),
        )
        data = QAResponseData(
            answer=answer,
        ...
        try:
            ...
            store.append(
                ...
                SessionTurn(
                    query=body.query,
                    answer=answer,
                    meta={"top_k": body.top_k, "n_sources": len(sources), "stream": True},
                ),
            )
...
def get_session(
    ...
    """Optional: inspect session summary (turns kept in memory)."""
    ...
    summary = {
        ...
        "turns": [
            {"query": t.query, "answer_preview": (t.answer or "")[:160]}
            for t in rec.turns
        ],
    }
    return success_response(summary, request_id=request.state.request_id)
